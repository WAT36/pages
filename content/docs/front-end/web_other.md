...
インターレースはGIF,PNGで使える。JPEGではインターレースににた「プログレッシブJPEG」を設定することができる。
...
コンバージョンレートとは、Webサイトへのアクセス数やユーザー数のうち、どの程度が収益に結びついたかを示す割合のことである。

ここでいう収益とは、商品購入や会員登録などのことを指す。（無論、定義はWebサイトによって異なる）


## ROI 

ROIは投資利益率(Realm On Inversion)の事で、投資した資本に対して得られる利益の割合のことである。