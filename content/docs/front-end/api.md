...
Service Workerと組み合わせることで、アプリケーションが動作しているかどうかにかかわらず受信できる。
...
WebRTC(Web Real-Time Communication)は、ブラウザでリアルタイムな子μy７ウニケーションを実現するための仕組みである。WebRTCを使うことで、ブラウザ間のビデオチャットやボイスチャット、会議システムなどが実現可能です。